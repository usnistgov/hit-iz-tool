...
@RestController
@RequestMapping("/transport/{domain}/soap")
...
public class SOAPTransportController {
	...
	public SOAPTransportController() throws IOException {
		InputStream templateStream = SOAPTransportController.class.getResourceAsStream("/templates/SubmitSingleMessage.xml");
		try {
			SUMBIT_SINGLE_MESSAGE_TEMPLATE = IOUtils.toString(templateStream);
		} finally {
	...
	private boolean clearExchanges(Long userId, String domain) {
		try {
			...
			// Clear messages and transactions
			...
			logger.error("Error during exchange cleanup", e);
			throw e;
		} finally {
			// Clear any thread-local resources
			ThreadLocal<TransportMessage> threadLocal = new ThreadLocal<>();
			threadLocal.remove();
		}
	}
	...
	public TransportConfig configs(@PathVariable("domain") String domain, @ApiParam(value = "The user session", required = true)  HttpSession session,
	...
	private Map<String, String> sutInitiatorConfig(Account user, HttpServletRequest request)
		...
		config.put("faultPassword",
				user.isGuestAccount() ? "fault_vendor_" + user.getId() + "_" + token : user.getUsername());
		config.put("endpoint", appInfoService.get().getUrl() + "ws/iisService");
		return config;
	}
